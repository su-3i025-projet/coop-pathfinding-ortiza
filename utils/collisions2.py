...
class CollisionHandler2:
    ...
    def __init__(self, screen, spritesize):
        ...
        self.fastGroupCollide = fast_rect_collision.FastGroupCollide(
            group={}, display_size=wh, max_interv=spritesize)
    ...
    def collision_list(self, s, group_filter=None):
        l = self.fastGroupCollide.compute_collision_list(
            s, pygame.sprite.collide_mask)
        return self._filter_by_layername(l, group_filter)

    def collision_blocking_player(self, s):
        blockinglayers = {'obstacle'} if self.allow_overlaping_players else {
            'obstacle', 'joueur'}
    ...
    def handle_collision(self, gDict, _safe_collision=True):
        ...
        multi_player_and_not_allow_overlap = len(
            persos) > 1 and not allow_overlap
        ...
        for j in persos:
            if _safe_collision:
                assert not self.collide_player_w_obstacles(
                    j, backup=True), "sprite collision with obstacles before any movement !!!"
                if multi_player_and_not_allow_overlap:
                    assert not self.collide_player_w_players(
                        j, backup=True), "sprite collision before any movement !!!"
        ...
        for j in persos:
            ...
            if multi_player_and_not_allow_overlap:
                self.erase_player_mask(j, backup=True)
        ...
        good_layernames = set(
            gDict) - {'bg1', 'bg2', 'dessinable', 'eye_candy'}
