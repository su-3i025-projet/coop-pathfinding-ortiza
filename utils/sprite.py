...
class MySprite(pygame.sprite.Sprite):
    ...
    def __init__(self, layername, tileid, x, y, imglist):
        ...
        # tileid identifie le sprite sur la spritesheet. Generalement, c'est le row/col dans le spritesheet
        self.tileid = tileid
    ...
    def get_rowcol(self):
        assert int(self.rect.x) % self.rect.w == 0 and int(
            self.rect.y) % self.rect.h == 0, "sprite must not be accross tiles for this function"
...
class MovingSprite(MySprite):
    ...
    def position_changed(self): return (
        self.backup_x, self.backup_y) != (self.x, self.y)
    ...
    def set_centroid(self, x, y):
        self.translate_sprite(x - self.rect.w // 2, y -
                              self.rect.h // 2, self.angle_degree, relative=False)
    ...
    def forward(self, t):
        dx, dy = cos(self.angle_degree * pi /
                     180), sin(self.angle_degree * pi / 180)
        if self.angle_degree % 90 == 0:
            dx, dy = round(dx), round(dy)
        self.translate_sprite(t * dx, t * dy, 0)

    def get_rowcol(self):
        assert int(self.x) % self.rect.w == 0 and int(
            self.y) % self.rect.h == 0, "sprite must not be accross tiles for this function"
        return int(self.y) // self.rect.h, int(self.x) // self.rect.w

    def set_rowcol(self, row, col):
        self.translate_sprite(col * self.rect.w, row *
                              self.rect.h, self.angle_degree, relative=False)
