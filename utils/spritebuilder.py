...
class SpriteBuilder(object):
    ...
    def __init__(self, file_name):
        ...
        # sprite size in pixels (assume its a square)
        self.spritesize = 0
        ...
        try:
            ...
            try:
                sheet_filename = dirname + "/Cartes/" + \
                    self.carte["tilesets"][0]["image"]
                self.sheet = SpriteSheet(sheet_filename, self.spritesize)
    ...
    def buildGroups(self):
        ...
        Grps = OrderedDict([(gr, self.basicGroupFactory(gr))
                            for gr in glo.ALL_LAYERS])
        ...
        for l in self.carte["layers"]:
            ...
            if "compression" in l:
                ...
                dat = map(lambda x: ord(
                    x[0]) + 256 * ord(x[1]) + 256**2 * ord(x[2]) + 256**3 * ord(x[3]), dat3)

            for idx, e in enumerate(dat):
                y, x = (idx // self.rowsize) * self.spritesize, (idx %
                                                                 self.rowsize) * self.spritesize
                # if (e > 0 and  64<=x<=200 and 64<=y<=200) or (layername=='joueur' and e > 0):
                if e > 0:
                    s = self.basicSpriteFactory(
                        layername, self.sheet.get_row_col(e - 1), x, y, self.sheet[e - 1])
