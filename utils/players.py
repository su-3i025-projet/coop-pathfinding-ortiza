...
class Player(MovingSprite):
    ...
    def gen_callbacks(self, incr, gDict, mask):
        ...
        return {
            ...
            pygame.K_t:     partial(
                self.throw_ray, radian_angle=None, mask=mask, layers=gDict)
        }
    ...
    def throw_rays(self, radian_angle_list, mask, layers, coords=None, show_rays=False):
        ...
        r = [rayon.rayon(mask.mask, cx, cy, a, w, h)
             for a in radian_angle_list]
        mask.draw_sprite(self)
        if layers and show_rays:
            for h in r:
                layers["eye_candy"].add(DrawOnceSprite(
                    pygame.draw.line, [(255, 0, 0), (cx, cy), h, 4]))
        return r
...
class Turtle(Player):
    def __init__(self, layername, x, y, w, h):
        ...
        Player.__init__(self, layername, tileid=None, x=x, y=y,
                        imglist=self.build_Turtle_list_images(w, h))
    ...
    def build_Turtle_list_images(self, w, h):
        ...
        for a, img in zip(range(360), imglist):
            ...
            img.fill((0, 0, 0))
            circle(img, glo.WHITE, (w / 2, h / 2),
                   self.taille_geometrique / 2 - self.penwidth, self.penwidth)
            polygons.draw_arrow(img, w / 2, h / 2, a * pi / 180,
                                r=self.taille_geometrique - 14, clr=glo.WHITE)
